...
rospy.init_node('AGV_teleop_key5')
pub_left=rospy.Publisher('/cmd_vel_left',Float64,queue_size=20)
...
def go_1ms():
    ...
    while(sec<=0.5):
	    ...
	    pub_left.publish(target_linear_vel) # /cmd_vel 메시지 퍼블리시
	    #pub_right.publish(target_linear_vel) # /cmd_vel 메시지 퍼블리시 
...
def turn():
	...
	key=getkey()
        ...
        sec=sec+0.05
        pub_left.publish(target_linear_vel)
	#pub_right.publish(-target_linear_vel)
	rospy.loginfo("Turn : %s" % target_linear_vel) 
	rate.sleep()
	if key == ' ' or key == 's' :
	    target_linear_vel=0.0
	    break
	...
	while(1):
		...
		target_linear_vel=0.0
                ...
                if key == '\x03': # ctrl + c 처리
                    break
	    pub_left.publish(target_linear_vel) #vel 값 publish하기
	    #pub_right.publish(target_linear_vel) #vel 값 publish하기
